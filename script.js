...
const superheroesArray = JSON.parse(superheroesJSON);

const superheroesDiv = document.querySelector('.superheroes');

superheroesArray.forEach(function (obj, i){
    superheroesDiv.innerHTML = superheroesDiv.innerHTML + `<div class="superhero">
    <h3>${obj.name}</h3>
    <p>Вселенная: ${obj.universe}</p>
    <p>Альтер эго: ${obj.alterego}</p>
    <p>Род деятельности: ${obj.occupation}</p>
    <p>Друзья: ${obj.friends}</p>
    <p>Суперсилы: ${obj.superpowers}</p>
    <div class="image"><img src="${obj.url}"></div>
    <p>${obj.info}</p>
    <div class="rating"  data-total-value="0" id="${i}">
...
rating.forEach(el1 => { 
    const id = `ID: ${el1.id}`;
    const localRatings = JSON.parse(localStorage.getItem(id));
    if(localRatings != null){ 
        el1.setAttribute("data-total-value", localRatings['data']);
    }
    const ratingItems = Array.from(el1.querySelectorAll('.rating__item'));
    ratingItems.forEach(el2 => { 
        el2.addEventListener('click', () => {
            const value = el2.getAttribute("data-item-value");
            el1.setAttribute("data-total-value", value);
